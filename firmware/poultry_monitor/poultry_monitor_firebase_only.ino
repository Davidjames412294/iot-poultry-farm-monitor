 ...
 *               Wi-Fi communication with Google Firebase for real-time monitoring.
 * ================================================================
 */
...
String checkThresholds() {
  ...
  if (!allNormal) {
    if (WiFi.status() == WL_CONNECTED) {
      ...
      Serial.println("No WiFi. Alert not sent to Firebase.");
    }
  }
  return sysStatus;
}
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("\nWi-Fi connection failed. Firebase unavailable.");
    lcd.clear();
    lcd.setCursor(0, 0);
...
void loop() {
  ...
  if (now - lastReadTime >= READ_INTERVAL) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      Serial.println("No WiFi. Firebase update skipped.");
    }
